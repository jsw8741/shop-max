...
public class ItemController {
	...
	public String itemShopList(Model model, ItemSearchDto itemSearchDto, Optional<Integer> page) {
		...
		Page<MainItemDto> items = itemService.mainItemPage(itemSearchDto, pageable);
	...
	public String itemMng(ItemSearchDto itemSearchDto, 
		...
		// of(조회할 페이지의 번호, 한페이지당 조회할 데이터 갯수)
		// url 경로에 페이지가 있으면 해당 페이지 번호를 조회하도록하고 페이지 번호가 없으면 0페이지를 조회
		...
		model.addAttribute("items", items);
		model.addAttribute("itemSearchDto", itemSearchDto);
		model.addAttribute("maxPage", 5); // 상품 관리 페이지 하단에 보여줄 최대 페이지 번호
